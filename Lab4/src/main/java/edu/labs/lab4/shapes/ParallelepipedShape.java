...
public class ParallelepipedShape extends Shape {
  ...
  private final LineShape[] lines;
  private final RectangleShape[] rectangles;
  private final GraphicsContext gc;

  public ParallelepipedShape(double startX, double startY, double endX, double endY,
      GraphicsContext gc) {
    ...
    rectangles[0] = new RectangleShape(startX - offset, startY - offset, startX + offset,
        startY + offset, gc);
    rectangles[1] = new RectangleShape(endX - offset, endY - offset, endX + offset, endY + offset,
        gc);

    // Initialize your lines here
    lines = new LineShape[4];
    lines[0] = new LineShape(rectangles[0].getLeft(), rectangles[0].getTop(),
        rectangles[1].getLeft(), rectangles[1].getTop(), gc);
    lines[1] = new LineShape(rectangles[0].getRight(), rectangles[0].getTop(),
        rectangles[1].getRight(), rectangles[1].getTop(), gc);
    lines[2] = new LineShape(rectangles[0].getLeft(), rectangles[0].getBottom(),
        rectangles[1].getLeft(), rectangles[1].getBottom(), gc);
    lines[3] = new LineShape(rectangles[0].getRight(), rectangles[0].getBottom(),
        rectangles[1].getRight(), rectangles[1].getBottom(), gc);
    this.gc = gc;
  }
